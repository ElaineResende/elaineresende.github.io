...
class Graficos3 extends React.Component {
  ...
  updateGraph = () => {
    ...
    const dt = uniqueKeys.map(u => ({
      description: u,
      value: ele.filter(e => e.cat_party === u).reduce((pv, v) => pv + parseInt(v.num_votes, 10), 0),
    })).filter(v => v.value !== 0);
  ...
  render() {
    ...
    return (
      <Card>
        <CardHeader
          title="Porcentagem de votos por partidos"
          ...
          <div className={classes.root} >
            {data && data.length > 0 &&
              <BarChart
                ...
                height={450}
                key={this.state.key}
                title={`Porcentagem de votos por partidos em ${this.state.uf === 'T' ? 'Brasil' : this.state.uf}`}
                source="Source: IBGE, 2014"
                xLabel="Partidos"
                yLabel="Porcentagem de votos"
                data={data}
                ceil={2}
              />
            }
...
export default withStyles(styles, { withTheme: true })(Graficos3);
