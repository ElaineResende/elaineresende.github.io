...
export class PieChart extends React.Component {
  ...
  componentDidMount() {
    ...
    const svg = select(node) // select element in the DOM with id 'chart' append an svg element to the element we've selected
      .attr('width', width) // set the width of the svg element we just added
      .attr('height', height) // set the height of the svg element we just added
      .append('g') // append 'g' element to the svg element
      .attr('transform', 'translate(' + (height / 2) + ',' + (height / 2) + ')'); // our reference is now to the 'g' element. centerting the 'g' element to the svg element
    ...
    // define tooltip
    const tooltip = select(node) // select element in the DOM with id 'chart'
    ...
    path.on('mouseover', function (d) {  // when mouse enters div      
      const total = d3.sum(dataset.map(function (d) { // calculate the total number of tickets in the dataset         
